...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def _read_filters_for_args(self, args: dict) -> Dict[str, Any]:
        filters = dict(self._read_filters())
        kind = args.get("kind")
        if kind:
            # Snapshot kinds are written by an external pipeline under the
            # configured user_id, not necessarily the gateway-derived one.
    ...
    def system_prompt_block(self) -> str:
        ...
        return (
            ...
            "mem0_list or mem0_search to target one of them directly."
        )
        ...
        if tool_name == "mem0_list":
            try:
                ...
                response = self._backend.get_all(
                    filters=self._read_filters_for_args(args), page=page, page_size=page_size,
                )
                self._record_success()
            ...
            try:
                ...
                results = self._backend.search(
                    query,
                    filters=self._read_filters_for_args(args),
                    top_k=top_k,
                    rerank=rerank,
                )
                self._record_success()
